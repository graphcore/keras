...
class NetworkConstructionTest(keras_parameterized.TestCase):
  def setUp(self):
    super(NetworkConstructionTest, self).setUp()  # pylint: disable=super-with-arguments
  ...
  def tearDown(self):
    ...
    super(NetworkConstructionTest, self).tearDown()  # pylint: disable=super-with-arguments
  ...
  def test_node_construction(self):
    ...
    a_layer, a_node_index, a_tensor_index = a._keras_history  # pylint: disable=protected-access
    b_layer, _, _ = b._keras_history  # pylint: disable=protected-access
    self.assertEqual(len(a_layer._inbound_nodes), 1)  # pylint: disable=protected-access
    self.assertEqual(a_tensor_index, 0)
    node = a_layer._inbound_nodes[a_node_index]  # pylint: disable=protected-access
    ...
    self.assertEqual(len(dense._inbound_nodes), 2)  # pylint: disable=protected-access
    self.assertEqual(len(dense._outbound_nodes), 0)  # pylint: disable=protected-access
    self.assertEqual(dense._inbound_nodes[0].inbound_layers, a_layer)  # pylint: disable=protected-access
    self.assertEqual(dense._inbound_nodes[0].outbound_layer, dense)  # pylint: disable=protected-access
    self.assertEqual(dense._inbound_nodes[1].inbound_layers, b_layer)  # pylint: disable=protected-access
    self.assertEqual(dense._inbound_nodes[1].outbound_layer, dense)  # pylint: disable=protected-access
    self.assertIs(dense._inbound_nodes[0].input_tensors, a)  # pylint: disable=protected-access
    self.assertIs(dense._inbound_nodes[1].input_tensors, b)  # pylint: disable=protected-access
  ...
  def test_multi_input_layer(self):
    with self.cached_session():
      ...
      merge_layer, merge_node_index, merge_tensor_index = merged._keras_history  # pylint: disable=protected-access

      self.assertEqual(merge_node_index, 0)
      self.assertEqual(merge_tensor_index, 0)

      self.assertEqual(len(merge_layer._inbound_nodes), 1)  # pylint: disable=protected-access
      self.assertEqual(len(merge_layer._outbound_nodes), 0)  # pylint: disable=protected-access

      self.assertEqual(len(merge_layer._inbound_nodes[0].input_tensors), 2)  # pylint: disable=protected-access
      self.assertEqual(len(merge_layer._inbound_nodes[0].inbound_layers), 2)  # pylint: disable=protected-access
      ...
      self.assertListEqual(model.compute_mask([a, b], [None, None]),
                           [None, None])
      ...
      self.assertListEqual(
          [l.name for l in model._input_layers],  # pylint: disable=protected-access
          ['input_a', 'input_b'])
      self.assertListEqual(
          [l.name for l in model._output_layers],  # pylint: disable=protected-access
          ['dense_2', 'dense_3'])
      ...
      self.assertListEqual([l.name for l in recreated_model.layers][2:],
                           ['dense_1', 'merge', 'dense_2', 'dense_3'])
      self.assertListEqual(
          [l.name for l in recreated_model._input_layers],  # pylint: disable=protected-access
          ['input_a', 'input_b'])
      self.assertListEqual(
          [l.name for l in recreated_model._output_layers],  # pylint: disable=protected-access
          ['dense_2', 'dense_3'])
  ...
  def test_recursion(self):
    with tf.Graph().as_default(), self.cached_session():
      ...
      final_model = training_lib.Model(inputs=[e, f],
                                       outputs=[i, g],
                                       name='final')
      ...
      self.assertListEqual([layer.name for layer in final_model.layers][2:],
                           ['model', 'dense_4'])
      self.assertListEqual(model.compute_mask([e, f], [None, None]),
                           [None, None])
      self.assertListEqual(
  ...
  def testMaskingSingleInput(self):
    ...
    if tf.executing_eagerly():
      ...
      a._keras_mask = mask  # pylint: disable=protected-access
      b = MaskedLayer().apply(a)
      self.assertTrue(hasattr(b, '_keras_mask'))
      self.assertAllEqual(self.evaluate(tf.ones_like(mask)),
  ...
  def test_layer_sharing_at_heterogenous_depth(self):
    x_val = np.random.random((10, 5)).astype(dtype=np.int32)
    ...
    output_val = m.predict(x_val, batch_size=5)
    ...
    output_val_2 = m2.predict(x_val, batch_size=5)
  ...
  def test_layer_sharing_at_heterogenous_depth_with_concat(self):
    ...
    x_val = np.random.random((10, 16, 9, 3)).astype(dtype=np.int32)
    output_val = m.predict(x_val, batch_size=5)
  ...
  def test_call_kwarg_derived_from_keras_layer(self, share_already_used_layer):
    class MaybeAdd(layers.Layer):
      def call(self, x1, x2=None):  # pylint: disable=arguments-differ
        if x2 is not None:
          return x1 + x2
        return x1
  ...
  def test_call_kwarg_derived_from_keras_layer_and_first_arg_is_constant(
    ...
    model.compile('sgd', 'mse')
    history = model.fit(x=7 * np.ones((10, 10)),
                        y=10 * np.ones((10, 10)),
                        batch_size=2)
    # Check that second input was correctly added to first.
    self.assertEqual(history.history['loss'][0], 0.0)
  ...
  def test_subclassed_error_if_init_not_called(self):
    class MyNetwork(training_lib.Model):  # pylint: disable=abstract-method
  ...
  def test_model_initialization(self):
    ...
    with self.assertRaisesRegex(TypeError, 'Keyword argument not understood'):
      model = training_lib.Model(inputs,
                                 outputs,
                                 name='m',
                                 trainable=False,
                                 dtype='int64')
    ...
    class SubclassModel(training_lib.Model):  # pylint: disable=abstract-method
      pass

    # Subclassed model
    model = SubclassModel(name='subclassed',
                          trainable=True,
                          dtype='int64',
                          dynamic=True)
  ...
  def test_dependency_tracking(self):
    ...
    self.assertIn('trackable', model._unconditional_dependency_names)  # pylint: disable=protected-access
    self.assertEqual(model.trackable, model._lookup_dependency('trackable'))  # pylint: disable=protected-access
...
class DeferredModeTest(keras_parameterized.TestCase):
  def setUp(self):
    super(DeferredModeTest, self).setUp()  # pylint: disable=super-with-arguments
    cfg = ipu.config.IPUConfig()
    cfg.auto_select_ipus = 1
    cfg.ipu_model.compile_ipu_code = False
    cfg.ipu_model.tiles_per_ipu = 1
  ...
  def tearDown(self):
    self._ipu_strategy_scope.__exit__(None, None, None)
    super(DeferredModeTest, self).tearDown()  # pylint: disable=super-with-arguments
...
class DefaultShapeInferenceBehaviorTest(keras_parameterized.TestCase):
  def setUp(self):
    super(DefaultShapeInferenceBehaviorTest, self).setUp()  # pylint: disable=super-with-arguments
    cfg = ipu.config.IPUConfig()
    cfg.auto_select_ipus = 1
    cfg.ipu_model.compile_ipu_code = False
    cfg.ipu_model.tiles_per_ipu = 1
  ...
  def tearDown(self):
    self._ipu_strategy_scope.__exit__(None, None, None)
    super(DefaultShapeInferenceBehaviorTest, self).tearDown()  # pylint: disable=super-with-arguments
  ...
  @combinations.generate(combinations.combine(mode=['graph']))
  def test_external_keras_serialization_compat_input_layers(self):  # pylint: disable=protected-access
  ...
  @combinations.generate(combinations.combine(mode=['graph']))
  def test_external_keras_serialization_compat_inbound_nodes(self):  # pylint: disable=protected-access
    # Check single Tensor input.
    inputs = input_layer_lib.Input(shape=(10,), name='in')
    outputs = layers.Dense(1)(inputs)
    model = training_lib.Model(inputs, outputs)
    config = model.get_config()
    self.assertEqual(config['layers'][1]['inbound_nodes'],
                     [[['in', 0, 0, {}]]])
...
class NestedNetworkTest(keras_parameterized.TestCase):
  def setUp(self):
    super(NestedNetworkTest, self).setUp()  # pylint: disable=super-with-arguments
    cfg = ipu.config.IPUConfig()
    cfg.auto_select_ipus = 1
    cfg.ipu_model.compile_ipu_code = False
    cfg.ipu_model.tiles_per_ipu = 1
  ...
  def tearDown(self):
    self._ipu_strategy_scope.__exit__(None, None, None)
    super(NestedNetworkTest, self).tearDown()  # pylint: disable=super-with-arguments
  ...
  def test_nested_dict_mapping(self):
    ...
    self.assertFalse(model._enable_dict_to_input_mapping)  # pylint: disable=protected-access
    self.assertEqual(self.evaluate(res), [1234])


class AddLossTest(keras_parameterized.TestCase):
  def setUp(self):
    super(AddLossTest, self).setUp()  # pylint: disable=super-with-arguments
    cfg = ipu.config.IPUConfig()
    cfg.auto_select_ipus = 1
    cfg.ipu_model.compile_ipu_code = False
    cfg.ipu_model.tiles_per_ipu = 1
  ...
  def tearDown(self):
    self._ipu_strategy_scope.__exit__(None, None, None)
    super(AddLossTest, self).tearDown()  # pylint: disable=super-with-arguments
...
class WeightAccessTest(keras_parameterized.TestCase):
  def setUp(self):
    super(WeightAccessTest, self).setUp()  # pylint: disable=super-with-arguments
    cfg = ipu.config.IPUConfig()
    cfg.auto_select_ipus = 1
    cfg.ipu_model.compile_ipu_code = False
    cfg.ipu_model.tiles_per_ipu = 1
  ...
  def tearDown(self):
    self._ipu_strategy_scope.__exit__(None, None, None)
    super(WeightAccessTest, self).tearDown()  # pylint: disable=super-with-arguments
  ...
  def test_subclass_model_with_build_method(self):
    class SubclassModel(models.Model):  # pylint: disable=abstract-method
      def build(self, input_shape):
...
class DTypeTest(keras_parameterized.TestCase):
  def setUp(self):
    super(DTypeTest, self).setUp()  # pylint: disable=super-with-arguments
    cfg = ipu.config.IPUConfig()
    cfg.auto_select_ipus = 1
    cfg.ipu_model.compile_ipu_code = False
    cfg.ipu_model.tiles_per_ipu = 1
  ...
  def tearDown(self):
    self._ipu_strategy_scope.__exit__(None, None, None)
    super(DTypeTest, self).tearDown()  # pylint: disable=super-with-arguments
  ...
  def test_subclassed_network_dtype(self):
    class IdentityNetwork(training_lib.Model):  # pylint: disable=abstract-method
      def call(self, inputs):  # pylint: disable=arguments-differ
        return inputs
...
class AttrTrackingLayer(base_layer.Layer):
  ...
  def stateful(self):  # pylint: disable=invalid-overridden-method
...
class CacheCorrectnessTest(keras_parameterized.TestCase):
  def setUp(self):
    super(CacheCorrectnessTest, self).setUp()  # pylint: disable=super-with-arguments
    cfg = ipu.config.IPUConfig()
    cfg.auto_select_ipus = 1
    cfg.ipu_model.compile_ipu_code = False
    cfg.ipu_model.tiles_per_ipu = 1
  ...
  def tearDown(self):
    self._ipu_strategy_scope.__exit__(None, None, None)
    super(CacheCorrectnessTest, self).tearDown()  # pylint: disable=super-with-arguments
  ...
  def test_training_passed_during_construction(self):
    ...
    class MyLayer(base_layer.Layer):
      def call(self, inputs, training=True):  # pylint: disable=arguments-differ
...
class InputsOutputsErrorTest(keras_parameterized.TestCase):
  def setUp(self):
    super(InputsOutputsErrorTest, self).setUp()  # pylint: disable=super-with-arguments
    cfg = ipu.config.IPUConfig()
    cfg.auto_select_ipus = 1
    cfg.ipu_model.compile_ipu_code = False
    cfg.ipu_model.tiles_per_ipu = 1
  ...
  def tearDown(self):
    self._ipu_strategy_scope.__exit__(None, None, None)
    super(InputsOutputsErrorTest, self).tearDown()  # pylint: disable=super-with-arguments
