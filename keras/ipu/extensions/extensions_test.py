    ...
    with strategy.scope():
      ...
      class MakeFunctionsOverrideModel(training_module.Model):  # pylint: disable=abstract-method
        def __init__(self, dense_layers):
          super(MakeFunctionsOverrideModel, self).__init__()  # pylint: disable=super-with-arguments
  ...
  def testInputValidation(self):
    class TwoInTwoOutIdentity(layers.Layer):
      def __init__(self, **layer_args):
        super(TwoInTwoOutIdentity, self).__init__(**layer_args)  # pylint: disable=super-with-arguments
    ...
    with strategy.scope():
      ...
      with extensions.PipelineStage(0):
        x = layers.Dense(10)(input_x)
      with extensions.PipelineStage(1):
        x = layers.Dense(1)(x)
