...
class PollingThread(threading.Thread):
  ...
  def _iterate_over_replica_results(self, data):  # pylint: disable=missing-yield-type-doc
    ...
    for replica in range(self._replication_factor):
      yield tf.nest.map_structure(lambda t: t[replica], data)  # pylint: disable=cell-var-from-loop
...
class PollingThreadPredict(PollingThread):
  ...
  def run(self):
    ...
    while step < end_step:
      ...
      flat_results = tf.nest.flatten(results)
      # Skip if no results are ready.
      if not flat_results:
        self.postprocess(0)
        continue

      num_iterations = tf.shape(flat_results[0]).numpy()[0]
      ...
      for iteration in range(num_iterations):
        ...
        for flat_data in self._iterate_over_replica_results(
          ...
          data = tf.nest.pack_sequence_as(results, flat_data)
      ...
      # Append the results.
      merged_results = extensions_util.merge_into_batch_dimension(
