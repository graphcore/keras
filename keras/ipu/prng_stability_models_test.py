...
class PrngStabilityModelsTest(tf.test.TestCase):
  @staticmethod
  ...
  def testSimpleLinearModel(self, stochastic_rounding_mode):
    ...
    with ipu.scopes.ipu_scope("/device:IPU:0"):
      ...
      def training_loop():
        def model(loss):
          x = tf.constant([[1], [2], [3], [4]], dtype=np.float16)
          y_true = tf.constant([[0], [-1], [-2], [-3]], dtype=np.float16)

          linear_model = keras.layers.Dense(units=1)

          y_pred = linear_model(x)
          loss = tf.compat.v1.losses.mean_squared_error(labels=y_true,
                                                        predictions=y_pred)
          ...
          with tf.control_dependencies([train]):
            ...
            return tf.cast(loss, dtype=np.float16)
    ...
    with ipu.scopes.ipu_scope("/device:IPU:0"):
      ...
      def training_loop():
        def model(loss):
          ...
          x = tf.cond(
              tf.equal(rep_id, 0),
              lambda: tf.constant([[1], [1], [1], [1]], dtype=np.float16),
              lambda: tf.constant([[1], [2], [3], [4]], dtype=np.float16))
          y_true = tf.constant([[0], [-1], [-2], [-3]], dtype=np.float16)
    ...
    with ipu.scopes.ipu_scope("/device:IPU:0"):
      ...
      def training_loop():
        def model(loss, images, labels):
          ...
          logits = keras.layers.Dense(10, activation='relu', name="dense3")(x)
    ...
    with ipu.scopes.ipu_scope("/device:IPU:0"):
      ...
      def model():
        ...
        def stage2(inputs, labels):
          logits = keras.layers.Dense(10, activation='relu',
                                      name="dense3")(inputs)
          cross_entropy = tf.nn.sparse_softmax_cross_entropy_with_logits(
              labels=labels, logits=logits)
          loss = tf.reduce_mean(cross_entropy)
...
@parameterized.named_parameters(*stochastic_rounding_modes())
class PrngStabilityModelsKerasTest(tf.test.TestCase):
  @staticmethod
